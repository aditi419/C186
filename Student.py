...
firebase = firebase.FirebaseApplication('https://p186-a6996-default-rtdb.firebaseio.com/',None)
...
def sendData():
    global username,message_entry,your_code
    message = username + ':' + message_entry.get()
    ciphercode = encrypt('AIM', message)
    hex_string = ciphercode.hex()
    putdata = firebase.put('/',your_code,hex_string)
    print(putdata)

def enterRoom():
    global username,your_code,your_friends_code,message_text,message_entry
    your_code = your_code_entry.get()
    your_friends_code = friend_code_entry.get()
